// Ported from AnyTorah/AnyTorah/Models/TextCatalog.swift — keep in sync with the Swift/Kotlin
// catalogs. Static reference data only; no logic beyond simple lookups.
...
export interface TanakhBook {
  id: number; // global index (0-38)
  name: string; // Hebrew transliterated display name
  hebrewName: string;
  sefariaName: string; // exact title used in Sefaria API refs (English)
  chapters: number;
}

// MARK: - Mishnah

export interface MishnahSeder {
  id: number;
  name: string;
  hebrewName: string;
  tractates: MishnahTractate[];
}

export interface MishnahTractate {
  id: number;
  name: string;
  hebrewName: string;
  sefariaName: string;
  chapters: number;
  toseftaChapters: number; // 0 = no Tosefta on Sefaria
  yerushalmiChapters: number; // 0 = not in Yerushalmi
}

// MARK: - Talmud

export interface TalmudSeder {
  id: number;
  name: string;
  hebrewName: string;
...
export interface TalmudTractate {
  ...
  endDaf: number;
  mishnahOnly: boolean;
  isYerushalmi: boolean;
}

// MARK: - Rambam (Mishneh Torah)

export interface RambamSefer {
  id: number;
  name: string;
  hebrewName: string;
  works: RambamWork[];
...
export interface RambamWork {
  id: number;
  name: string; // Hebrew transliterated display name
  hebrewName: string;
  sefariaName: string; // exact Sefaria title, e.g. "Mishneh Torah, Sabbath"
  chapters: number;
}
...
function book(
  ...
  return { id, name, hebrewName: he, sefariaName: sefaria, chapters: ch };
}

const torahSection: TanakhSection = {
  name: "Torah",
  hebrewName: "תורה",
  books: [
...
const neviiimSection: TanakhSection = {
  name: "Nevi'im",
  hebrewName: "נְבִיאִים",
  books: [
...
const ketuvimSection: TanakhSection = {
  name: "Ketuvim",
  hebrewName: "כְּתוּבִים",
  books: [
...
export const allTanakhBooks: TanakhBook[] = tanakhSections.flatMap(
  (s) => s.books,
);
...
function mt(
  ...
  return {
    ...
    sefariaName: sefaria,
    chapters: ch,
    toseftaChapters: tosefta,
    yerushalmiChapters: yerushalmi,
  };
}

const zeraimSeder: MishnahSeder = {
  id: 0,
  name: "Zeraim",
  hebrewName: "זְרָעִים",
  tractates: [
    mt(0, "Berakhot", "בְּרָכוֹת", "Mishnah Berakhot", 9, 7, 9),
    mt(1, "Peah", "פֵּאָה", "Mishnah Peah", 8, 4, 8),
    mt(2, "Demai", "דְּמַאי", "Mishnah Demai", 7, 8, 7),
    mt(3, "Kilayim", "כִּלְאַיִם", "Mishnah Kilayim", 9, 5, 9),
    mt(4, "Sheviit", "שְׁבִיעִית", "Mishnah Sheviit", 10, 8, 10),
    mt(5, "Terumot", "תְּרוּמוֹת", "Mishnah Terumot", 11, 10, 11),
    mt(6, "Maasrot", "מַעַשְׂרוֹת", "Mishnah Maasrot", 5, 3, 5),
    mt(7, "Maaser Sheni", "מַעֲשֵׂר שֵׁנִי", "Mishnah Maaser Sheni", 5, 5, 5),
    mt(8, "Challah", "חַלָּה", "Mishnah Challah", 4, 2, 4),
    mt(9, "Orlah", "עָרְלָה", "Mishnah Orlah", 3, 1, 3),
    mt(10, "Bikkurim", "בִּכּוּרִים", "Mishnah Bikkurim", 4, 2, 3),
  ],
};

const moedSeder: MishnahSeder = {
  id: 1,
  name: "Moed",
  hebrewName: "מוֹעֵד",
  tractates: [
    mt(11, "Shabbat", "שַׁבָּת", "Mishnah Shabbat", 24, 18, 24),
    mt(12, "Eruvin", "עֵרוּבִין", "Mishnah Eruvin", 10, 8, 10),
    mt(13, "Pesachim", "פְּסָחִים", "Mishnah Pesachim", 10, 10, 10),
    mt(14, "Shekalim", "שְׁקָלִים", "Mishnah Shekalim", 8, 3, 8),
    mt(15, "Yoma", "יוֹמָא", "Mishnah Yoma", 8, 4, 8),
    mt(16, "Sukkah", "סוּכָּה", "Mishnah Sukkah", 5, 4, 5),
    mt(17, "Beitzah", "בֵּיצָה", "Mishnah Beitzah", 5, 4, 5),
    mt(18, "Rosh Hashanah", "רֹאשׁ הַשָּׁנָה", "Mishnah Rosh Hashanah", 4, 2, 4),
    mt(19, "Taanit", "תַּעֲנִית", "Mishnah Taanit", 4, 3, 4),
    mt(20, "Megillah", "מְגִלָּה", "Mishnah Megillah", 4, 3, 4),
    mt(21, "Moed Katan", "מוֹעֵד קָטָן", "Mishnah Moed Katan", 3, 2, 3),
    mt(22, "Chagigah", "חֲגִיגָה", "Mishnah Chagigah", 3, 3, 3),
  ],
};

const nashimSeder: MishnahSeder = {
  id: 2,
  name: "Nashim",
  hebrewName: "נָשִׁים",
  tractates: [
    mt(23, "Yevamot", "יְבָמוֹת", "Mishnah Yevamot", 16, 14, 16),
    mt(24, "Ketubot", "כְּתֻבּוֹת", "Mishnah Ketubot", 13, 12, 13),
    mt(25, "Nedarim", "נְדָרִים", "Mishnah Nedarim", 11, 7, 11),
    mt(26, "Nazir", "נָזִיר", "Mishnah Nazir", 9, 6, 9),
    mt(27, "Sotah", "סוֹטָה", "Mishnah Sotah", 9, 15, 9),
    mt(28, "Gittin", "גִּיטִּין", "Mishnah Gittin", 9, 7, 9),
    mt(29, "Kiddushin", "קִדּוּשִׁין", "Mishnah Kiddushin", 4, 5, 4),
  ],
};

const nezikinSeder: MishnahSeder = {
  id: 3,
  name: "Nezikin",
  hebrewName: "נְזִיקִין",
  tractates: [
    mt(30, "Bava Kamma", "בָּבָא קַמָּא", "Mishnah Bava Kamma", 10, 11, 10),
    mt(31, "Bava Metzia", "בָּבָא מְצִיעָא", "Mishnah Bava Metzia", 10, 11, 10),
    mt(32, "Bava Batra", "בָּבָא בַּתְרָא", "Mishnah Bava Batra", 10, 11, 10),
    mt(33, "Sanhedrin", "סַנְהֶדְרִין", "Mishnah Sanhedrin", 11, 14, 11),
    mt(34, "Makkot", "מַכּוֹת", "Mishnah Makkot", 3, 4, 3),
    mt(35, "Shevuot", "שְׁבוּעוֹת", "Mishnah Shevuot", 8, 6, 8),
    mt(36, "Eduyot", "עֵדֻיּוֹת", "Mishnah Eduyot", 8, 3, 0),
    mt(37, "Avodah Zarah", "עֲבוֹדָה זָרָה", "Mishnah Avodah Zarah", 5, 9, 5),
    mt(38, "Pirkei Avot", "פִּרְקֵי אָבוֹת", "Pirkei Avot", 6, 0, 0),
    mt(39, "Horayot", "הוֹרָיּוֹת", "Mishnah Horayot", 3, 2, 3),
  ],
};

const kodashimSeder: MishnahSeder = {
  id: 4,
  name: "Kodashim",
  hebrewName: "קֳדָשִׁים",
  tractates: [
    mt(40, "Zevachim", "זְבָחִים", "Mishnah Zevachim", 14, 13),
    mt(41, "Menachot", "מְנָחוֹת", "Mishnah Menachot", 13, 13),
    mt(42, "Chullin", "חוּלִּין", "Mishnah Chullin", 12, 10),
    mt(43, "Bekhorot", "בְּכוֹרוֹת", "Mishnah Bekhorot", 9, 7),
    mt(44, "Arakhin", "עֲרָכִין", "Mishnah Arakhin", 9, 5),
...
const taharotSeder: MishnahSeder = {
  id: 5,
  name: "Taharot",
  hebrewName: "טָהֳרוֹת",
  tractates: [
    mt(51, "Keilim", "כֵּלִים", "Mishnah Kelim", 30),
    mt(52, "Ohalot", "אֳהָלוֹת", "Mishnah Ohalot", 18, 18),
    mt(53, "Negaim", "נְגָעִים", "Mishnah Negaim", 14, 9),
    mt(54, "Parah", "פָּרָה", "Mishnah Parah", 12, 12),
    mt(55, "Taharot", "טָהֳרוֹת", "Mishnah Taharot", 10),
    mt(56, "Mikvaot", "מִקְוָאוֹת", "Mishnah Mikvaot", 10, 8),
    mt(57, "Niddah", "נִדָּה", "Mishnah Niddah", 10, 9),
    mt(58, "Makhshirin", "מַכְשִׁירִין", "Mishnah Makhshirin", 6, 3),
    mt(59, "Zavim", "זָבִים", "Mishnah Zavim", 5, 5),
    mt(60, "Tevul Yom", "טְבוּל יוֹם", "Mishnah Tevul Yom", 4, 2),
    mt(61, "Yadayim", "יָדַיִם", "Mishnah Yadayim", 4, 2),
    mt(62, "Uktzin", "עֻקְצִין", "Mishnah Oktzin", 3, 3),
  ],
};

export const mishnahSedarim: MishnahSeder[] = [
  zeraimSeder,
  moedSeder,
  nashimSeder,
  nezikinSeder,
...
export const allMishnahTractates: MishnahTractate[] = mishnahSedarim.flatMap(
  (s) => s.tractates,
);
...
function tt(
  ...
  mishnahOnly: boolean = false,
  isYerushalmi: boolean = false,
): TalmudTractate {
  return {
    id,
    name,
...
const zeraimT: TalmudSeder = {
  id: 0,
  name: "Zeraim",
  hebrewName: "זְרָעִים",
  tractates: [tt(0, "Berakhot", "בְּרָכוֹת", "Berakhot", 2, 64)],
};

const moedT: TalmudSeder = {
  id: 1,
  name: "Moed",
  hebrewName: "מוֹעֵד",
  tractates: [
    ...
    tt(4, "Shekalim", "שְׁקָלִים", "Shekalim", 2, 22, false, true),
...
const taharotT: TalmudSeder = {
  id: 5,
  name: "Taharot",
  hebrewName: "טָהֳרוֹת",
  tractates: [tt(39, "Niddah", "נִדָּה", "Niddah", 2, 73)],
};

export const talmudSedarim: TalmudSeder[] = [
  zeraimT,
  moedT,
  nashimT,
  nezikinT,
...
export const allTalmudTractates: TalmudTractate[] = talmudSedarim.flatMap(
  (s) => s.tractates,
);
...
export const rambamSefarim: RambamSefer[] = [
  {
    id: 0,
    name: "HaMadda",
    hebrewName: "סֵפֶר הַמַּדָּע",
    works: [
...
export const allRambamWorks: RambamWork[] = rambamSefarim.flatMap(
  (s) => s.works,
);
...
export const shulchanArukhSections: ShulchanArukhSection[] = [
  { id: 0, name: "Orach Chayim", hebrewName: "אוֹרַח חַיִּים", sefariaName: "Shulchan Arukh, Orach Chayim", simanim: 697 },
  { id: 1, name: "Yoreh Deah", hebrewName: "יוֹרֶה דֵּעָה", sefariaName: "Shulchan Arukh, Yoreh De'ah", simanim: 403 },
  { id: 2, name: "Even HaEzer", hebrewName: "אֶבֶן הָעֵזֶר", sefariaName: "Shulchan Arukh, Even HaEzer", simanim: 178 },
