...
export default function SASimanPicker({
  ...
  const topicSections = getSATopicSections(section);
          ...
          {topicSections.map((topic) => (
            <div key={topic.name} className="mb-3">
              <div className="sticky top-0 bg-card px-2 py-1 text-xs font-medium opacity-50">
                {topic.name} ({topic.start}
                {topic.end !== topic.start ? `–${topic.end}` : ""})
              </div>
              <div>
                {Array.from({ length: topic.end - topic.start + 1 }, (_, i) => topic.start + i).map(
                  (siman) => {
                    const title = getSASimanTitle(section, siman);
                    ...
                    return (
                      ...
                      >
                        <span className="w-8 shrink-0 text-right tabular-nums opacity-70">{siman}</span>
