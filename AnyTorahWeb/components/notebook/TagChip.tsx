 ...
 *  the exact chip's DOM position (extractTags in lib/notebooks.ts). */
export default function TagChip({ node, deleteNode }: NodeViewProps) {
  return (
    <NodeViewWrapper as="span" id={node.attrs.tagId} className="notebook-tag-chip" contentEditable={false}>
      🏷 {node.attrs.label}
