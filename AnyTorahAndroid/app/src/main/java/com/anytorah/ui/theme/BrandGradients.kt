 ...
 * AnyYCTorah's exact hex values (see [prefersDarkForeground] below). [PLUM], [TEAL], and [NAVY]
 * are new same-style extensions sized to exactly the ten home-screen categories here (five
 * purple-family, five blue-family) — not claimed as confirmed YCT brand hexes, matching how
 * AnyYCTorah's own non-brand extensions (green/gold/lavender/blossom) are documented there.
 */
enum class BrandColorFamily(private val stops: List<Color>, val prefersDarkForeground: Boolean = false) {
    ...
    LAVENDER(listOf(Color(0xFFE0C2FA), Color(0xFFBA7FEB), Color(0xFF6A2FB0)), prefersDarkForeground = true),
    BLOSSOM(listOf(Color(0xFFF9D4FB), Color(0xFFEE9CF7), Color(0xFFB84FC4)), prefersDarkForeground = true),
