 ...
 * Tosefta; Tur/Shulkhan Arukh) and shares its color rather than getting its own — so this enum
 * now has only 6 cases, not one per category. LAVENDER/BLOSSOM/TEAL/NAVY were retired across
 * that change and an earlier one (tile text standardized to always-white, removing the need for
 * LAVENDER/BLOSSOM's pale, dark-foreground-friendly hues; the right column's colors shifting
 * down by one, retiring NAVY) — no category needs a color of its own anymore.
 */
